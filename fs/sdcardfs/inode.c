...
#if 0
static int
sdcardfs_setxattr(struct dentry *dentry, const char *name, const void *value,
...
static ssize_t
sdcardfs_getxattr(struct dentry *dentry, const char *name, void *buffer,
		size_t size)
{
	int err;
	struct dentry *lower_dentry;
...
static ssize_t
sdcardfs_listxattr(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	int err;
	struct dentry *lower_dentry;
	struct path lower_path;
