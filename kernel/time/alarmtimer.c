...
static int alarm_poweroff(struct device *dev)
{
	int rc = 0;
	struct rtc_wkalrm   rtc_alarm;
	struct rtc_time     rtc_current_rtc_time;
	...
	if (rc)
		printk("alarm_poweroff Unable to set power-on alarm\n");
	else
		printk("alarm_poweroff Power-on alarm set to %lu\n",
				rtc_alarm_time);
	}else if(1 == qpnp_acorusb_pw)
    {
        ...
        {
            printk("alarm_poweroff qpnp_acorusb_pw=%d goto disable_alarm \n",qpnp_acorusb_pw);
            goto disable_alarm;
        }
    }
    else
...
disable_alarm:
    printk("alarm_poweroff disable_alarm \r\n");
    rtc_alarm_time = 0;
    rtc_time_to_tm(rtc_alarm_time, &rtc_alarm.time);
    rtc_alarm.enabled = 0;
	...
	if (rc){
		printk("alarm_poweroff disable_alarm fail to set power-on alarm rc=%d \n",rc);
    }else{
		printk("alarm_poweroff disable_alarm ok Power-on alarm set to %lu\n",
				rtc_alarm_time);
    }
	mutex_unlock(&power_on_alarm_lock);
    return 0;
