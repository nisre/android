...
/*lenovo.sw.begin chenyb1 20140919 . boost cpu interface for alarm wakeup */
int lenovo_get_cpufreq_suspend(void)
{
    int device_suspended = 0;
    
    device_suspended = per_cpu(cpufreq_suspend, 0).device_suspended;

    return device_suspended;
}
/*lenovo.sw.end chenyb1 20140919 . boost cpu interface for alarm wakeup */
