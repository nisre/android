...
/* -------------------- Lenovo Edit Begin -------------------- */
// liubin modified 2014.07.23, optimize CPU scheduling strategy for Bluetooth A2DP. see PCR-6 for details
static ssize_t show_mpctl(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, MAX_LONG_SIZE, "%u\n", rq_info.mpctl);
}

static ssize_t store_mpctl(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	sscanf(buf, "%u", &rq_info.mpctl);
	sysfs_notify(rq_info.kobj, NULL, "mpctl");
	return count;
}

static struct kobj_attribute mpctl_attr = __ATTR(mpctl, S_IWUSR | S_IRUSR, show_mpctl, store_mpctl);
/* -------------------- Lenovo Edit Ebd -------------------- */
