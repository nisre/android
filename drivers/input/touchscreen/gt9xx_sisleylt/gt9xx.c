...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    {  
		...
		if(type == 0){
			...
			if(tap_type[2] == 2){	
				...
				letter = 0x24;
                		GTP_INFO("Gesture---->Double click to light up the screen!");
    ...
    if (ret < 0)
    {
        ...
        if (ts->use_irq)
        {
            //GTP_INFO("ahe work gtp_irq_enable 22");
            gtp_irq_enable(ts);
        }
        return;
    }
...
power_off:
    ...
    if(gtp_fw_version_num > 0x42){//pvt close L6
	    ...
	    if (!IS_ERR(ts->vcc_i2c)) {
            ...
            dev_err(&ts->client->dev,
				    "ahe regulator_disable vcc_i2c  ret=%d\n", ret);
		    if (ret)
			    dev_err(&ts->client->dev,
	...
	if (!IS_ERR(ts->vdd)) {
		ret = regulator_disable(ts->vdd);
        dev_err(&ts->client->dev,
				"ahe regulator_disable vdd  ret=%d\n", ret);
		if (ret)
			dev_err(&ts->client->dev,
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   // spin_lock_init(&ts->esd_lock);
	...
	dev_err(&client->dev, "ahe reset done \n");
    if (ret < 0)
    {
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
...
#endif //0
#ifndef GTP_CUT_RESUME_TIME
    GTP_INFO("ahe  resume &&&&&& 111111.");
    ret = gtp_wakeup_sleep(ts);
    GTP_INFO("ahe  resume &&&&&& 222222.");
#if GTP_GESTURE_WAKEUP
	if(gtp_wakeup_flag)
    		doze_status = DOZE_DISABLED;
#endif
    ...
    else
    {
	 ...
	 GTP_INFO("ahe  resume &&&&&& 444444.");
    ts->gtp_is_suspend = 0;
#if GTP_ESD_PROTECT
    gtp_esd_switch(ts->client, SWITCH_ON);
#endif
	 GTP_INFO("ahe  resume &&&&&& 5555555.");
#else
