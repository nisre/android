...
static int actuator_cci_inited = 0;
/*lenovo-sw add end*/
...
static int msm_actuator_init(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->act_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
		...
		/*lenovo-sw chenglong1 add for af standby*/
		actuator_cci_inited = 1;
		pr_err("cci_init success actuator_cci_inited=%d\n", actuator_cci_inited);
		/*lenovo-sw add end*/
	}
...
static int msm_actuator_close(struct v4l2_subdev *sd,
	...
	/*lenovo-sw chenglong1 add for af standby*/
	if ((a_ctrl->act_device_type == MSM_CAMERA_PLATFORM_DEVICE) && (!actuator_cci_inited)) {
		pr_err("[AF Debug] cci init for unbalanced release now...\n");
		if (!msm_actuator_init(a_ctrl)) {
			pr_err("[AF Debug] sent actuator to sleep\n");
			msm_actuator_standby(a_ctrl);
		}
	}
	/*lenovo-sw add end*/
