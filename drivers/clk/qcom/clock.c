...
int clk_enable(struct clk *clk)
{
	...
	if (IS_ERR(clk))
		return -EINVAL;
	name = clk->dbg_name;
	/* chenyb1 20140514. Add to debug bb_clk2 active beforesleep begin */
	if ( !strcmp((const char *)clk->dbg_name, "bb_clk2"))
	{
		printk("bb_clk2 debug:%s\n", __func__);
		dump_stack();
	}
	/* chenyb1 20140514. Add to debug bb_clk2 active beforesleep end */
