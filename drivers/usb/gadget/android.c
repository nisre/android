...
static bool android_init = false;
void popup_usb_select_window(int popup){
	...
	static int old_state = 0;
        //lenovo-sw yexh1 add for [FRIESIAN-9048], which may cause usb cannot send UNAVAILABLE in racing condition
        bool ignore = false;

	if (dev)
		mutex_lock(&dev->mutex);

	if(android_init == false){
		ignore = true;
	}
...
// which will cause phone crash PC is at collect_langs. 
	//yexh1 struct android_dev *dev = ffs_function.android_dev;
        struct android_dev *dev = _android_dev;
//lenovo.sw yexh1 end
...
struct android_dev *_android_dev = NULL;
