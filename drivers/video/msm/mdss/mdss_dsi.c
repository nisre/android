...
#include <soc/qcom/socinfo.h> //lenovo.sw2 houdz1 add for of_board_is_sisley()
...
struct delayed_work lcd_color_work;
int color_enable = 0;
DEFINE_MUTEX(color_lock);
/*char buf[2];*/
...
void color_timer_handle(unsigned long data);
struct mdss_panel_data *pdata_esd = NULL;
extern void mdss_dsi_panel_color_dcs(struct mdss_dsi_ctrl_pdata *ctrl);
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	if (pdata->panel_info.panel_power_on) {
		...
		if (ctrl_pdata->lcd_id) {
			printk("============ enable color work ==============\n");
			color_enable = 1;
			schedule_delayed_work(&lcd_color_work, DELAY_TIME);
		}
#endif
		return 0;
	}
	 ...
	 */
	if (mipi->lp11_init) {
		if(of_board_is_z2()||of_board_is_sisleyl()) pdata->panel_info.panel_power_on = 0; //lenovo.sw2 houdz1 add for Z2 lcd power on timming
		else
		{
			if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
				pr_debug("reset enable: pinctrl not enabled\n");
		...
		if(of_board_is_z2()||of_board_is_sisleyl()) pdata->panel_info.panel_power_on = 1;//lenovo.sw2 houdz1 add for Z2 lcd power on timming
	}
...
extern int lenovo_lcd_effect_handle(struct mdss_dsi_ctrl_pdata *ctrl_pdata,void *ctrl_data);
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_LENOVO_LCD_EFFECT
		ctrl_pdata->lenovo_lcdEffect_handle =  lenovo_lcd_effect_handle;
...
static void lcd_color_abnormal_handle(struct work_struct *work)
{
    struct mdss_dsi_ctrl_pdata *ctrl = NULL;

    if(pdata_esd == NULL)
    {
        pr_err("==jinjt==pdata_esd=NULL\n");
		return;
    }
	ctrl = container_of(pdata_esd, struct mdss_dsi_ctrl_pdata, panel_data);
    if(ctrl == NULL)
    {
        pr_err("==jinjt==ctrl=NULL\n");
		return;
    }
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->lcd_enp_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,platform-enp-gpio", 0);
	pr_err("%s, lcd enp gpio [%d]\n",__func__,ctrl_pdata->lcd_enp_gpio);
	if (!gpio_is_valid(ctrl_pdata->lcd_enp_gpio))
		pr_err("%s:%d, lcd enp gpio not specified\n",
				__func__, __LINE__);
	ctrl_pdata->lcd_enn_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,platform-enn-gpio", 0);
	pr_err("%s, lcd enn gpio [%d]\n",__func__,ctrl_pdata->lcd_enn_gpio);
	if (!gpio_is_valid(ctrl_pdata->lcd_enn_gpio))
		pr_err("%s:%d, lcd enn gpio not specified\n",
				__func__, __LINE__);
...
#endif
	ctrl_pdata->disp_id_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,platform-id-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->disp_id_gpio)) {
		pr_err("%s:%d, Disp_id gpio not specified\n",
				__func__, __LINE__);
	}

	else{
		rc = gpio_request(ctrl_pdata->disp_id_gpio, "disp_id");
		if (rc) {
			pr_err("request id gpio failed, rc=%d\n",rc);
		}
		rc = gpio_get_value(ctrl_pdata->disp_id_gpio);
		ctrl_pdata->lcd_id = rc;
		printk("=============== id: %d ================\n", rc);
	}
...
#endif
/*lenovo.sw2 houdz1 add(start): add "disp_vsp_gpio" and "disp_vsn_gpio"*/ 
	ctrl_pdata->disp_vsp_gpio= of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-vsp-gpio", 0);
	pr_err("%s, lcd vsp gpio [%d]\n",__func__,ctrl_pdata->disp_vsp_gpio);
	...
	pr_err("%s, lcd vsn gpio [%d]\n",__func__,ctrl_pdata->disp_vsn_gpio);
	if (!gpio_is_valid(ctrl_pdata->disp_vsn_gpio))
		pr_err("%s:%d, lcd vsn gpio not specified\n",__func__, __LINE__);
	else
	{
...
/*lenovo.sw2 houdz1 add(end): add "disp_vsp_gpio" and "disp_vsn_gpio"*/ 
...
/*lenovo.sw2 houdz1 add for sisleyl:ultra mode(end)*/
    ...
    INIT_DELAYED_WORK(&lcd_color_work, lcd_color_abnormal_handle);
#endif
